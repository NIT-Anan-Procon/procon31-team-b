...
class Player{
    ...
    move(){
        ...
        let x_right = (this.x + this.r) | 0;    //円の右端の座標
        let x_left = (this.x - this.r) | 0;     //円の左端の座標
        let y_high = (this.y - this.r) | 0;     //円の上端の座標
        let y_low = (this.y + this.r) | 0;      //円の下端の座標

        //方向キーが押され、道(塗りつぶさないところ)の場合、playerが移動する
        if(input.up){
            y_high = (this.y - this.r - this.speed) | 0;
        ...
        if(input.down){
            y_low = (this.y + this.r+this.speed) | 0;
        ...
        if(input.right){
            x_right = (this.x + this.r+this.speed) | 0;
        ...
        if(input.left){
            x_left = (this.x - this.r-this.speed) | 0;
...
// プレイヤーのオブジェクトを作成
let player = new Player(ctx, 2.5, 1, 0.4, 'blue');
